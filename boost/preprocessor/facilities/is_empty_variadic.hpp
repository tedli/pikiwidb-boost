...
#define BOOST_PP_IS_EMPTY(param)                                               \
  BOOST_PP_DETAIL_IS_EMPTY_IIF(BOOST_PP_IS_BEGIN_PARENS(param))                \
  (BOOST_PP_IS_EMPTY_ZERO, BOOST_PP_DETAIL_IS_EMPTY_PROCESS)(param) /**/
#define BOOST_PP_IS_EMPTY_ZERO(param) 0
#else
#if defined(__cplusplus) && __cplusplus > 201703L
#include <boost/preprocessor/variadic/has_opt.hpp>
#define BOOST_PP_IS_EMPTY(...)                                                 \
  BOOST_PP_DETAIL_IS_EMPTY_IIF(BOOST_PP_VARIADIC_HAS_OPT())                    \
  (BOOST_PP_IS_EMPTY_OPT, BOOST_PP_IS_EMPTY_NO_OPT)(__VA_ARGS__) /**/
...
#define BOOST_PP_IS_EMPTY_NO_OPT(...)                                          \
  BOOST_PP_DETAIL_IS_EMPTY_IIF(BOOST_PP_IS_BEGIN_PARENS(__VA_ARGS__))          \
  (BOOST_PP_IS_EMPTY_ZERO, BOOST_PP_DETAIL_IS_EMPTY_PROCESS)(__VA_ARGS__) /**/
